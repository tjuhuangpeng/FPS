...
public class AHFloatingMenu extends ViewGroup {
    ...
    private AHAddFloatingButton mAddButton;
    ...
    private OnFloatingMenuListener mListener;
    ...
    public AHFloatingMenu(Context context) {
        this(context, null);
    }

    public AHFloatingMenu(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public AHFloatingMenu(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void init(Context context, AttributeSet attributeSet) {
        ...
        TypedArray attr = context.obtainStyledAttributes(attributeSet, R.styleable.AHFloatingMenu, 0, 0);
        mAddButtonPlusColor = attr.getColor(R.styleable.AHFloatingMenu_fab_addButtonPlusIconColor, getColor(android.R.color.white));
        mAddButtonColorNormal = attr.getColor(R.styleable.AHFloatingMenu_fab_addButtonColorNormal, getColor(android.R.color.holo_blue_dark));
        mAddButtonColorPressed = attr.getColor(R.styleable.AHFloatingMenu_fab_addButtonColorPressed, getColor(android.R.color.holo_blue_light));
        mAddButtonSize = attr.getInt(R.styleable.AHFloatingMenu_fab_addButtonSize, AHFloatingButton.SIZE_NORMAL);
        mAddButtonStrokeVisible = attr.getBoolean(R.styleable.AHFloatingMenu_fab_addButtonStrokeVisible, true);
        mExpandDirection = attr.getInt(R.styleable.AHFloatingMenu_fab_expandDirection, EXPAND_UP);
        mLabelsStyle = attr.getResourceId(R.styleable.AHFloatingMenu_fab_labelStyle, 0);
        mLabelsPosition = attr.getInt(R.styleable.AHFloatingMenu_fab_labelsPosition, LABELS_ON_LEFT_SIDE);
        attr.recycle();
    ...
    /**
     * 标签所在子菜单位置
     * LABELS_ON_LEFT_SIDE = 0;
     * LABELS_ON_RIGHT_SIDE = 1;
     */
    public void setLabelsPosition(int side) {
        switch (side) {
            case LABELS_ON_RIGHT_SIDE:
                mLabelsPosition = LABELS_ON_RIGHT_SIDE;
                break;
            default:
    ...
    public void setLabelsStyle(int styleId) {
        mLabelsStyle = styleId;
        if (mLabelsStyle != 0) {
            createLabels();
        }
    }
    ...
    public void setAddButtonStrokeVisible(boolean isVisible) {
        mAddButtonStrokeVisible = isVisible;
        mAddButton.setStrokeVisible(mAddButtonStrokeVisible);
    }

    public void setAddButtonSize(@AHFloatingButton.FAB_SIZE int size) {
        if (size != AHFloatingButton.SIZE_MINI && size != AHFloatingButton.SIZE_NORMAL) {
    ...
    public void setAddButtonPlusIconColor(int color) {
    ...
    public void setOnFloatingActionsMenuUpdateListener(OnFloatingMenuListener listener) {
        mListener = listener;
    }
    ...
    /**
     * 创建浮层RootMenu,注意,该方法只能调用一次
     */
    private void createAddButton(Context context) {
        mAddButton = new AHAddFloatingButton(context) {
            @Override
    ...
    public void addButton(AHFloatingButton button) {
    ...
    public boolean removeButton(AHFloatingButton button) {
        int count = getChildCount();
        boolean isRemove = false;
        for(int i=0;i<count;i++){
            if(getChildAt(i)==button){
                isRemove = true;
                break;
            }
        }
    ...
    private void createLabels() {
        ...
        for (int i = 0; i < mButtonsCount; i++) {
            AHFloatingButton button = (AHFloatingButton) getChildAt(i);
